...
public class CPeer extends Thread 
{
    ...
    CPeer(CPeers peers, String adr, int port) throws Exception
    {
	   ...
	   this.peers=peers;
           ...
           {
              // Address
              InetSocketAddress address = new InetSocketAddress(InetAddress.getByName(adr), port);
	   ...
	   { 
               // Log
	       System.out.println(ex.getMessage() + " - CPeer.java, 107"); 
   ...
   {  
       ...
       {  
           ...
           {
               // Aluve
               if (Math.abs(alive-UTILS.BASIC.tstamp())>5 
                   && alive>0)
               peers.removePeer(parent);
